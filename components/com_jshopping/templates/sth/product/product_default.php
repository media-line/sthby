...
$doc->addScript('/components/com_jshopping/js/functions.js');
unset($doc);

// ID характеристики товара для кноки "Почитать об этом товаре"
$readAbout = 1;

$language = JFactory::getLanguage();
$languageTag = $language->getTag();
$shortDescriptionField = 'short_description_'.$languageTag;
    ...
    <form name="product" method="post" action="<?php print $this->action?>" enctype="multipart/form-data" autocomplete="off">
		...
		<div class="uk-grid">
			<div class="uk-width-2-3">
				<h1 class="uk-text-bold uk-product-full-title uk-margin-small-top uk-margin-bottom-remove"><?php print $this->product->name?><?php if ($this->config->show_product_code){?> <span class="jshop_code_prod">(<?php print _JSHOP_EAN?>: <span id="product_code"><?php print $this->product->getEan();?></span>)</span><?php }?></h1>
				<div class="uk-product-full-shortdesc">
					<?php echo $this->product->{$shortDescriptionField}; ?>
				</div>
			</div>
			<div class="uk-width-1-3 uk-text-right">
				<?php if (is_array($this->product->extra_field)){?>
				<?php// print_r($this->product->extra_field);?>
					<?php foreach($this->product->extra_field as $extra_field){?>
						<?php if ($extra_field['id'] == $readAbout){?>
							<a class="uk-button uk-button-small uk-read-about uk-position-relative" href="<?php echo $extra_field['value']?>">
								<span><?php echo $extra_field['name']?></span>
							</a>
						<?php }?>
					<?php }?>
				<?php }?>
			...
			<div class="uk-width-1-4 uk-product-full-image-block">
				<div class="image_middle">
					...
					<div id='list_product_image_middle'>
						...
						<a class="uk-pruduct-main-image uk-text-center lightbox" id="main_image_full_<?php print $image->image_id?>" href="<?php print $this->image_product_path?>/<?php print $image->image_full;?>" <?php if ($k!=0){?>style="display:none"<?php }?> <?php /*title="<?php print htmlspecialchars($image->_title)?>"*/?> data-uk-lightbox="{group:'product-images'}">
				...
				<div class = "jshop_img_description">
					...
					<?php // по 3 картинки в ряд
						$thumbsPerRow = 3;
					?>
					
					<?php if ( (count($this->images)>1) || (count($this->videos) && count($this->images)) ) {?>
						<div id='list_product_image_thumb' class="uk-product-thumbs<?php if(count($this->images) == ($thumbsPerRow)) echo ' uk-thumbs-nomargin'?>">
							<?php foreach($this->images as $k=>$image){
								//if ($k > 0){?>
									<img id="<?php print $image->image_id?>" class="uk-product-thumb uk-jshop-img-thumb<?php if((($k) % $thumbsPerRow) == 0) echo ' uk-margin-left-remove';?>" src="<?php print $this->image_product_path?>/<?php print $image->image_thumb?>" alt="<?php print htmlspecialchars($image->_title)?>" title="<?php print htmlspecialchars($image->_title)?>" onclick="showImage(<?php print $image->image_id?>)" />
								<?php// }?>
							<?php }?>
						</div>	
					<?php }?>
		...
		<?php print $this->_tmp_product_html_before_atributes;?>
			...
			<?php// print_r($this->attributes);?>
			<?php// print_r($this->all_attr_values);?>
			<?php if (count($this->attributes)) : ?>
				<div class="uk-product-attr-title uk-h2 uk-text-bold"><?php echo JText::_("CATALOG_ATTRIBUTES_TITLE"); ?></div>
				<div class="uk-product-attributes uk-grid uk-grid-collapse">
					<?php// print_r($this->attributes);?>
					<?php foreach($this->attributes as $attribut) : ?>
						<?php/* if ($attribut->grshow){?>
							<div class="uk-product-attr-group uk-h2 uk-text-bold"><?php print $attribut->groupname?>
							</div>
						<?php }*/?>               
						<div class="uk-width-1-3">
							<div class="uk-text-center uk-attributes-head">
								<span class="uk-attributes-name"><?php print $attribut->attr_name?></span>
								<span class="attributes_description"><?php print $attribut->attr_description;?></span>
							</div>
							<div class = "">
							
								<div class="uk-attr-column uk-text-center" id='block_attr_sel_<?php print $attribut->attr_id?>'>
		...
		<div class="uk-grid">
			...
			<?php if (!$this->hide_buy){?>                         
				<div class="prod_buttons uk-width-2-3 uk-text-right" style="<?php print $this->displaybuttons?>">
					<div class="uk-quantity-block uk-inline-block uk-margin-large-right">
						<label class="uk-quantity-label" for="quantity"><?php print _JSHOP_QUANTITY?>:</label>
						<input type="text" name="quantity" id="quantity" onkeyup="reloadPrices();" class="uk-quantity-input" value="<?php print $this->default_count_product?>" /><?php print $this->_tmp_qty_unit;?>
					</div>
							
					<div class="uk-inline-block">            
						<div class="buttons">            
							<input type="submit" class="uk-button" value="<?php print _JSHOP_ADD_TO_CART?>" onclick="jQuery('#to').val('cart');" />
							
							<?php if ($this->enable_wishlist){?>
								<input type="submit" class="uk-button" value="<?php print _JSHOP_ADD_TO_WISHLIST?>" onclick="jQuery('#to').val('wishlist');" />
							<?php }?>
							
							<?php print $this->_tmp_product_html_buttons;?>
						</div>
						
						<?php /*<div id="jshop_image_loading" style="display:none"></div>*/ ?>
					</div>
				</div>
			<?php }?>
